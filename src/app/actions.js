...
export async function manageStaffRecord({ name, action, role }) {
  ...
  let isSenior = action === 'Promote' ? 'TRUE' : (action === 'Demote' ? 'FALSE' : (role === 'Senior' ? 'TRUE' : 'FALSE'));
  
  const rows = await rosterTab.getRows();
  const row = rows.find(r => r.get('Name')?.toLowerCase().trim() === name.toLowerCase().trim());
  
  if (row) {
    row.set('Support', isSupport); row.set('SeniorSupport', isSenior);
    await row.save();
  } else if (action === 'Add') {
    await rosterTab.addRow({ 'Name': name, 'Support': isSupport, 'SeniorSupport': isSenior });
  }
...
export async function issueStrike({ name, date, amount }) {
  ...
  // Find the exact row for that month to append the strike
